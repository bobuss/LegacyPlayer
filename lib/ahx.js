...
// this class is adapted from CODEF project
class dataType {
    ...
    readByteAt(atPos) {
        return this.data[atPos]
    }

    readStringAt(atPos) {
        var tmp = "";
        while (1) {
            if (this.data[atPos] != undefined && this.data[atPos++] != 0)
                tmp += String.fromCharCode(this.data[atPos - 1]);
            else
                return tmp;
        }
    }
...
class AHXSong {
    ...
    InitSong(data) {
        const stream = new dataType(data);
        stream.pos = 3;
...
class AHXWaves {
    ...
    Filter(input, fre, lowOrHigh) { // 0 = low, 1 = high
    ...
    ProcessFrame(v) {
        ...
        if (this.Voices[v].Instrument && this.Voices[v].PerfCurrent < this.Voices[v].Instrument.PList.Length) {
            if (--this.Voices[v].PerfWait <= 0) {
                ...
                for (let i = 0; i < 2; i++) this.PListCommandParse(v, this.Voices[v].PerfList.Entries[Cur].FX[i], this.Voices[v].PerfList.Entries[Cur].FXParam[i]);
                //GetNote
