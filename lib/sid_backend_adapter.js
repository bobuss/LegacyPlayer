/*
 sid_backend_adapter.js: Adapts Tiny'R'Sid backend to generic WebAudio/ScriptProcessor player.
...
const backend_module = backend_sid();
...
class SIDBackendAdapter extends EmsHEAP16BackendAdapter {
    constructor(basicROM, charROM, kernalROM, nextFrameCB) {
        ...
        this.setFilterConfig6581(
            filterWebSid.base,
            filterWebSid.max,
            filterWebSid.steepness,
            filterWebSid.x_offset,
            filterWebSid.distort,
            filterWebSid.distortOffset,
            filterWebSid.distortScale,
            filterWebSid.distortThreshold,
            filterWebSid.kink,
        );
    }
    ...
    */
    ...
    updateDigiMeta() {
        ...
        var dTypeStr= this.UTF8ToString(this.Module.ccall('getDigiTypeDesc', 'number'));
    ...
    loadMusicData(sampleRate, path, filename, data, options) {
        ...
        this.playerSampleRate = sampleRate;
    ...
    evalTrackOptions(options) {
        ...
        var procBufSize = 1024 //ScriptNodePlayer.getInstance().getScriptProcessorBufSize();
    ...
    updateSongInfo(filename) {
        const numAttr= 7;
        const ret = this.Module.ccall('getMusicInfo', 'number');
        const array = this.Module.HEAP32.subarray(ret>>2, (ret>>2)+7);

        this.songInfo.loadAddr= this.Module.HEAP32[((array[0])>>2)]; // i32
        this.songInfo.playSpeed= this.Module.HEAP32[((array[1])>>2)]; // i32
        this.songInfo.maxSubsong= this.Module.HEAP8[(array[2])]; // i8
        this.songInfo.actualSubsong= this.Module.HEAP8[(array[3])]; // i8
        this.songInfo.songName= this.UTF8ToString(array[4]);
        this.songInfo.songAuthor= this.UTF8ToString(array[5]);
        this.songInfo.songReleased= this.UTF8ToString(array[6]);
    ...
    */
    ...
    getTraceStreams() {
        ...
        for (var i= 0; i<n; i++) {
            this.songInfo.push(array[i] >> 1);    // pointer to int16 array
        }
        return result;
    }
