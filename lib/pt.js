...
function Protracker() {
    ...
    for (let t = 0; t < 16; t++) this.finetunetable[t] = Math.pow(2, (t - 8) / 12 / 8);
...
Protracker.prototype.parse = function (buffer) {
    ...
    for (let i = 0; i < 4; i++) this.signature += String.fromCharCode(buffer[1080 + i]);
    switch (this.signature) {
        ...
        default:
            this.signature = "";
            for (let i = 0; i < 4; i++) this.signature += String.fromCharCode(buffer[952 + i]);
    ...
    for (let i = 0; i < this.samples; i++) {
        ...
        if (this.signature === "")
            // loop start is a byte for old soundtracker mods
            this.sample[i].loopstart = (buffer[st + 26] * 256 + buffer[st + 27]);
        else
